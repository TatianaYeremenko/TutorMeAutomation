...
it.each < {
    ...
    async ({
        ...
        // await struct.account.profile.firstName.press('Enter');
        // await struct.account.profile.firstNameError.waitForVisible();
        // expect(await struct.account.profile.firstNameError.text()).toBe('This field is required.');

        // //first name invalid 
        // await struct.account.profile.firstName.clear();
        // await struct.account.profile.firstName.type('M m');
        // // await page.waitForTimeout(1000);
        // await struct.account.profile.firstNameError.waitForVisible();
        // expect(await struct.account.profile.firstNameError.text()).toBe('This field includes letters only.');
        ...
        await struct.account.profile.life.select.press('ArrowDown');
        await struct.account.profile.life.select.press('ArrowDown');
        await struct.account.profile.life.select.press('Enter');
        ...
        expect(await struct.account.profile.phoneError.text()).toBe('Mobile phone number format is not valid.');
        ...
        expect(await struct.account.profile.phone.value()).toBe('6502241234'.toString());
