...
describe("E2E Tutor with Writing in Subjects: ", () => {
 jest.setTimeout(1000000);

  const userFirstName = faker.name.firstName(0);
  const userLastName = faker.name.lastName();
  const email = `${userFirstName}${userLastName}tutor.ws@local.tutorme.com`;
  const password = "AllSubjects1234!";

  it("Fill out Tutor Application for AllSubjects", async () => {
  ...
  it("Tutor is able to login and switch to Tutor Mode", async () => {
    const { struct, page } = await createVisitor();
  ...
  it("Check Tutor Dashboard", async () => {
    ...
    // Select Writing as a subject
    await struct.userMenu.myAccount.waitForVisible();
    await struct.userMenu.myAccount.click();

    // click on Subjects tab
    await page.waitForSelector('//a[contains(text(),"Subjects")]');
    await (await page.waitForSelector('//a[contains(text(),"Subjects")]')).click();

    // click on Humanities 
    await struct.account.subjects.category(5).waitForVisible();
    await struct.account.subjects.category(5).click();

    // click on Writing 
    await struct.account.subjects.subject(142).waitForVisible();
    await struct.account.subjects.subject(142).click();
    ...
    //SCHECK IF WL links become AVAILABLE:
    ...
    await struct.tutorDashboard.resources.handbookWritingLab.waitForVisible();   
