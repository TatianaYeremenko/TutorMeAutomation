...
describe("E2E FPT: ", () => {
 jest.setTimeout(1000000);
  ...
  const email = `${userFirstName}${userLastName}tutor.fpt@local.tutorme.com`;
  const password = "Fpt12345!";

  it("Fill out Tutor Application for FPT", async () => {
  ...
  it("FP tutor is able to login and switch to Tutor Mode", async () => {
    const { struct, page } = await createVisitor();
  ...
  it("The tutor is checked as FP", async () => {
    //create Admin
    const a = await createAdmin();
    await a.page.waitForTimeout(1000);
    await a.page.keyboard.down("PageDown");
    ...
    await a.page.getByRole('listitem').filter({ hasText: 'FingerprintTrait' }).getByRole('checkbox').check();
