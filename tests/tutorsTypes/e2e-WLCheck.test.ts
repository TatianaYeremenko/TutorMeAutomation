...
describe("E2E WLcheck: ", () => {
 jest.setTimeout(1000000);

  const userFirstName = faker.name.firstName(0);
  const userLastName = faker.name.lastName();
  const email = `${userFirstName}${userLastName}tutor.wl@local.tutorme.com`;
  const password = "WLcheck1234!";

  it("Fill out Tutor Application for WLcheck", async () => {
  ...
  it("WLcheck tutor is able to login and switch to Tutor Mode", async () => {
    const { struct, page } = await createVisitor();
  ...
  it("The tutor is checked as WLcheck", async () => {
    //create Admin
    const a = await createAdmin();
    await a.page.waitForTimeout(1000);
    await a.page.keyboard.down("PageDown");
    ...
    // select type
    await a.page.getByRole('listitem').filter({ hasText: 'WritingLabTrait' }).getByRole('checkbox').check();
