...
describe("E2E NoConcierge: ", () => {
 jest.setTimeout(1000000);

  const userFirstName = faker.name.firstName(0);
  const userLastName = faker.name.lastName();
  const email = `${userFirstName}${userLastName}tutor.nocon@local.tutorme.com`;
  const password = "NoConcierge1234!";

  it("Fill out Tutor Application for NoConcierge", async () => {
  ...
  it("NoConcierge tutor is able to login and switch to Tutor Mode", async () => {
    const { struct, page } = await createVisitor();
  ...
  it("The tutor is checked as NoConcierge", async () => {
    //create Admin
    const a = await createAdmin();
    await a.page.waitForTimeout(1000);
    await a.page.keyboard.down("PageDown");
    ...
    // select type
    await a.page.getByRole('listitem').filter({ hasText: 'NoConciergeTrait' }).getByRole('checkbox').check();
