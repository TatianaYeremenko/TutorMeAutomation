...
describe("E2E NoLiveLessons: ", () => {
 jest.setTimeout(1000000);

  const userFirstName = faker.name.firstName(0);
  const userLastName = faker.name.lastName();
  const email = `${userFirstName}${userLastName}tutor.noll@local.tutorme.com`;
  const password = "NoLiveLessons1234!";

  it("Fill out Tutor Application for NoLiveLessons", async () => {
  ...
  it("NoLiveLessons tutor is able to login and switch to Tutor Mode", async () => {
    const { struct, page } = await createVisitor();
  ...
  it("The tutor is checked as NoLiveLessons", async () => {
    //create Admin
    const a = await createAdmin();
    await a.page.waitForTimeout(1000);
    await a.page.keyboard.down("PageDown");
    ...
    // select type
    await a.page.getByRole('listitem').filter({ hasText: 'NoLiveLessonsTrait' }).getByRole('checkbox').check();
  ...
  it("Check Tutor Dashboard", async () => {
    ...
    // past tutoring, earnings, heatmap, background check and resorcies should be avalable
    ...
    // availableTutoring should not
    await struct.tutorDashboard.header.availableTutoring.waitForHidden();
