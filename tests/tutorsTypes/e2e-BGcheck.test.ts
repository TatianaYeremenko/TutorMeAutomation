...
describe("E2E BGcheck: ", () => {
 jest.setTimeout(1000000);
  ...
  const email = `${userFirstName}${userLastName}tutor.bg@local.tutorme.com`;
  const password = "BGcheck1234!";

  it("Fill out Tutor Application for BGcheck", async () => {
  ...
  it("BGcheck tutor is able to login and switch to Tutor Mode", async () => {
    const { struct, page } = await createVisitor();
  ...
  it("BGcheck is selected in Admin Panel", async () => {
    //create Admin
    const a = await createAdmin();
    await a.page.waitForTimeout(1000);
    await a.page.keyboard.down("PageDown");
    ...
    await a.page.getByRole('listitem').filter({ hasText: 'BackgroundCheckTrait' }).getByRole('checkbox').check();
