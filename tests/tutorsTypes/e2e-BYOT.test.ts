...
describe("E2E BYOT: ", () => {
 jest.setTimeout(1000000);
  ...
  const email = `${userFirstName}${userLastName}tutor.byot@local.tutorme.com`;
  const password = "Byot1234!";

  it("Fill out Tutor Application to become BYOT", async () => {
  ...
  it("BYOT is able to login and switch to Tutor Mode", async () => {
    const { struct, page } = await createVisitor();
  ...
  it("The tutor is assigned to BYOT school", async () => {
    //create Admin
    const a = await createAdmin();
    await a.page.waitForTimeout(1000);
    await a.page.keyboard.down("PageDown");
    ...
    await (
      await a.page.waitForSelector(
        '//span[@aria-labelledby="select2-id_school_tutor_for-container"]'
      )
    ).click();
    await (
    ...
    await (
      await a.page.waitForSelector('//li[contains(text(),"BYOT")]')
    ).press("Enter");
  ...
  it("Tutor login again, check Tutor's Dashboard", async () => {
    const { struct, page } = await createVisitor();
    ...
    await struct.authPages.signIn.email.fill('robertemardtutor.byot@local.tutorme.com');
