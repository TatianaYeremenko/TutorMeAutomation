...
describe("E2E VIPkids: ", () => {
 jest.setTimeout(1000000);
  ...
  const email = `${userFirstName}${userLastName}tutor.vip@local.tutorme.com`;
  const password = "VIPkids1234!";

  it("Fill out Tutor Application for VIPkids", async () => {
  ...
  it("VIPkids tutor is able to login and switch to Tutor Mode", async () => {
    const { struct, page } = await createVisitor();
  ...
  it("The tutor is checked as VIPKids", async () => {
    //create Admin
    const a = await createAdmin();
    await a.page.waitForTimeout(1000);
    await a.page.keyboard.down("PageDown");
    ...
    await a.page.getByRole('listitem').filter({ hasText: 'VIPKidTrait' }).getByRole('checkbox').check();    await a.page.waitForTimeout(200);
  ...
  it("Check VIPkids Dashboard Setup", async () => {
    const { struct, page } = await createVisitor();
    ...
    await struct.authPages.signIn.email.waitForVisible();
    await struct.authPages.signIn.email.type(email.toLowerCase());
    // await struct.authPages.signIn.email.type('hectorgulgowskitutor.vip@local.tutorme.com');
    
    await struct.authPages.signIn.password.waitForVisible();
    await struct.authPages.signIn.password.fill(password);
    // await struct.authPages.signIn.password.fill('VIPkids1234!');
   ...
   // my account and notification should be there
    ...
    await (await page.waitForSelector('//h2[contains(text(),"Desktop notifications")]')).waitForElementState('visible');
